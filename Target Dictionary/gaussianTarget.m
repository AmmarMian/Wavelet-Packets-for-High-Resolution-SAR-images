...
%% ------------- BEGIN CODE --------------
function ImageTarget = gaussianTarget(x, y, kx, ky, position, Args)
...
%    - Args = {mean, scale} = means of gaussian pdf, scale of gaussian pdf
% Outputs:
...
%
    ...
    theta_0 = mean(2);
    sigma_theta = scale(2);
    gaussTerm = exp( -(k-k_0).^2/(2*sigma_k^2) ) .* exp( -(theta-theta_0).^2/(2*sigma_theta^2) );
    ImageTarget = ifft2(gaussTerm.*phaseTerm);
end
